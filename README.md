...
local distanciaSegura = 45          -- Altura para flutuar acima do monstro
local tempoAtaqueM1 = 0.15         -- Intervalo entre os tiros normais
local usarSkills = true            -- Ativa o uso das habilidades Z e X

local jogador = game.Players.LocalPlayer

-- Função para encontrar e equipar QUALQUER arma de fogo disponível
local function equiparQualquerGun()
    local personagem = jogador.Character
    local mochila = jogador:FindFirstChild("Backpack")
    
    if not personagem or not mochila then return nil end
    
    -- 1. Verifica se o jogador já está segurando alguma ferramenta
    local ferramentaAtual = personagem:FindFirstChildOfClass("Tool")
    if ferramentaAtual then
        -- Se já tiver uma ferramenta na mão, assume que é a arma a ser usada
        return ferramentaAtual
    end
    
    -- 2. Procura na mochila por qualquer ferramenta disponível
    -- Como o script é genérico, ele vai equipar a primeira ferramenta que encontrar
    local ferramentaEncontrada = mochila:FindFirstChildOfClass("Tool")
    if ferramentaEncontrada then
        ferramentaEncontrada.Parent = personagem
        return ferramentaEncontrada
    end
    
    return nil
end

-- Função de posicionamento aéreo e ataque direcionado
local function farmarEstiloVideoGenerico(inimigo)
    local personagem = jogador.Character
    local rootPart = personagem and personagem:FindFirstChild("HumanoidRootPart")
    
    if inimigo and inimigo:FindFirstChild("HumanoidRootPart") and inimigo.Humanoid.Health > 0 then
        local targetRoot = inimigo.HumanoidRootPart
        
        -- Loop de ataque enquanto o monstro estiver vivo
        while inimigo:FindFirstChild("Humanoid") and inimigo.Humanoid.Health > 0 do
            task.wait()
            
            -- Tenta equipar qualquer arma disponível
            local armaEquipada = equiparQualquerGun()
            
            -- Calcula a posição segura no ar acima do monstro
            local posicaoSegura = targetRoot.Position + Vector3.new(0, distanciaSegura, 0)
            
            if rootPart then
                -- Move o CFrame para flutuar e mirar exatamente no inimigo abaixo
                rootPart.CFrame = CFrame.new(posicaoSegura, targetRoot.Position)
                -- Mantém o personagem estável no ar anulando a gravidade física
                rootPart.Velocity = Vector3.new(0, 0, 0)
            end
            
            -- Se uma arma foi equipada com sucesso, executa o disparo
            if armaEquipada then
                armaEquipada:Activate()
            end
            task.wait(tempoAtaqueM1)
        end
    end
end

-- Função principal de monitoramento e busca no mar
function iniciarFarmUniversalGun()
    print("[MÉTODO UNIVERSAL] Procurando monstros no mar...")
    
    while true do
        task.wait(0.5)
        
        local filhosWorkspace = workspace:GetChildren()
        for _, objeto in pairs(filhosWorkspace) do
            -- Procura pelos alvos geradores de materiais para a Shark Anchor
            if objeto.Name == "Terror Shark" or objeto.Name == "Piranha" or objeto.Name == "Shark" then
                farmarEstiloVideoGenerico(objeto)
                print("[!] " .. objeto.Name .. " derrotado com sucesso.")
                
            -- Suporte para barcos e navios (Ouro Tolo)
            elseif objeto.Name == "Ship" or objeto.Name == "Ghost Ship" then
                local rootPart = jogador.Character and jogador.Character:FindFirstChild("HumanoidRootPart")
                if rootPart and objeto:FindFirstChild("PrimaryPart") then
                    -- Posiciona ao lado e acima do navio para disparar com segurança
                    rootPart.CFrame = CFrame.new(objeto.PrimaryPart.Position + Vector3.new(0, 30, 40), objeto.PrimaryPart.Position)
                    local armaEquipada = equiparQualquerGun()
                    if armaEquipada then 
                        armaEquipada:Activate() 
                    end
...
-- Para iniciar no executor:
-- task.spawn(iniciarFarmUniversalGun)
